...
**Granularité 3 niveaux** (docs concaténées, global → spécifique) : `docs/mcp/{server}.md` → `docs/mcp/{server}/{tool}.md` (suffixe après `mcp__{server}__`) → `docs/mcp/{server}/{subTool}.md` (paramètre, via `servers.{server}.subToolParam` = chemin pointé dans `tool_input`, ex. `"args.tool"` pour Odoo dont `tool_name` reste toujours `odoo_call`). Sans `subToolParam` configuré, niveau 3 inactif — pas de faux positif.
**Purge `state/`** : probabiliste (`~1/50` invocations, évite un `readdir`/`stat` à chaque appel), TTL 30 jours par défaut (`mtime`). Réglable via `MCP_DOC_GC_PROBABILITY`/`MCP_DOC_GC_TTL_MS` — usage TESTS uniquement, jamais en prod normale.
