---
rules: [{"pattern":"lib-pure.property.test.js","scope":["mcp-doc-hooks"],"rank":358},{"pattern":"lib-pure.js","scope":["mcp-doc-hooks"],"rank":363},{"pattern":"lib-pure.test.js","scope":["mcp-doc-hooks"],"rank":364}]
mode: dumb
confirm: true
rank: 358
---
...
Avant de "protéger" un cas avec une garde `if`, vérifier qu'elle n'est pas déjà couverte par la coercion JS native (regex `.exec()` coerce déjà les falsy) — une garde redondante = mutant Stryker équivalent, à éviter par construction plutôt qu'accepté.
`docCandidatePaths()` retourne des CANDIDATS (chemins calculés), jamais une lecture disque — le caller I/O (`mcp-doc-inject.js`) filtre ceux qui existent vraiment.
