...
⚠️ **SURVEILLÉ PAR LE DOCTOR depuis le 05/08/2026** (probe 9 + check câblage + 3 negative-checks). Il a tourné 2 jours en PROD sans aucune sonde : un dead-man switch que personne ne regarde est PIRE que rien, il fabrique de la confiance. La sonde prouve les DEUX verdicts (mort ET vivant) — un canari figé sur une constante passerait un test à un seul cas.
