...
🔴 **RÉSERVE, LA SEULE QUI RESTE : la porte SESSION (`session-inject.js`) n'a PAS de transport** — pas de morcelage, pas de sceau, pas de file. Elle tient parce que `docs/session/` pèse ~1,2 Ko ; au-delà, spill silencieux. Cause racine = le transport est un choix d'appelant et non une COUCHE (REFACTOR-PLAN ⑯ : extraire `emission-core.js` + gate dérivé, AVANT ⑮). 🛑 Ne pas dire « tout arrive partout » tant que ⑯ n'est pas fait.
**Sur la voie PreToolUse (docs fichier, docs MCP, skills), en revanche, l'indélivrabilité est impossible SANS RÉSERVE :** une doc de n'importe quelle taille arrive — 10 Ko, 80 Ko, peu importe. **NE JAMAIS réintroduire un plafond de taille, un « trop gros », un « scinde ta doc ».** Ce serait faire porter à l'AUTEUR d'une doc un défaut du TRANSPORT. Le framework livre, il ne juge pas ce qu'on lui confie. C'est pour ça que le volet ④ du `couverture-gate` (plafond de longueur) a été SUPPRIMÉ, et que la règle « <10 lignes » n'est qu'une convention de parc — jamais une contrainte du moteur.
