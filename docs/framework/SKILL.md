...
- `Desktop/mcp-doc-hooks/source-drift-gate.test.js` — GATE STATIQUE anti-dérive des 2 sources : `protected-paths.json` (ancien) ⟺ frontmatters du parc (nouveau), diff symétrique = 0 sur les 586 règles. Sens critique (SEULEMENT-JSON = neuf RATE = régression) distingué du bénin. Né 17/07/2026 (3 dérives de double écriture trouvées la veille de la bascule). Skippé sur clone vierge.
...
- `Desktop/mcp-doc-hooks/loader-differential.test.js` — DIFFÉRENTIEL in-process : règles reconstruites des frontmatters ≡ protected-paths.json (docs identiques ET ordonnées). Tourne à CHAQUE npm test (zéro spawn). Skippé sur clone vierge.
...
⚠️ TRANSITOIRE restant : `protected-paths.json` = fallback rollback (double écriture encore exigée par source-drift-gate). Son retrait définitif = seul geste optionnel restant, à froid.
