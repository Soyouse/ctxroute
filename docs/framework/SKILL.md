...
- **L'INJECTION informe par DÉFAUT** ; `enforce: true` est l'EXCEPTION DÉCLARÉE qui refuse le geste (05/08/2026). ⚠️ **Le fait qui a rouvert le sujet** : doc officielle, l'`additionalContext` d'un PreToolUse arrive *« next to the tool result »* ⇒ **une injection ne peut PAS empêcher le geste qu'elle vise**, elle protège le suivant. L'incident FONDATEUR (clic de paiement) n'aurait donc PAS été évité par une doc injectée — seul un refus le fait. `deny` = même JSON sur les 2 harnais, **zéro interaction utilisateur** (*« fully automatic »*), vérifié DANS le binaire Codex 0.144.6. `ask` PROSCRIT (escalade humaine = anti 0-human). 🛑 **Ce n'est PAS une sécurité mais un GARDE-FOU** : la porte est fail-open, un hook mort laisse passer — protège d'un agent distrait, jamais d'un adversaire.
