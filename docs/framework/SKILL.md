...
⚠️ **UNE SEULE INDÉLIVRABILITÉ SUBSISTE, mesurée le 05/08/2026** : la capacité d'UN appel est FINIE (N trames déclarées × 7 658 c = **91 896 c** au câblage actuel) — au-delà, le reliquat est ANNONCÉ puis abandonné, alors que RFC 2046/6455/TCP le DIFFÈRERAIENT. Trou réel, cible écrite (REFACTOR-PLAN ⑧ : file persistée émise aux appels suivants). 🛑 Ne plus affirmer « il livre TOUT » sans cette réserve. **Sous ce plafond, en revanche, l'indélivrabilité EST impossible par construction :** une doc de n'importe quelle taille arrive — 10 Ko, 80 Ko, peu importe. **NE JAMAIS réintroduire un plafond de taille, un « trop gros », un « scinde ta doc ».** Ce serait faire porter à l'AUTEUR d'une doc un défaut du TRANSPORT. Le framework livre, il ne juge pas ce qu'on lui confie. C'est pour ça que le volet ④ du `couverture-gate` (plafond de longueur) a été SUPPRIMÉ, et que la règle « <10 lignes » n'est qu'une convention de parc — jamais une contrainte du moteur.
