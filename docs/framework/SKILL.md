...
7bis. **SYMÉTRIE PAR DÉFAUT, EXCEPTION DÉCLARÉE (05/08/2026)** — toute clé de COMPORTEMENT vit dans les **4 corpus** (doc fichier · doc MCP · entrée skill · `defaults.{source}`) ; un décalage doit être JUSTIFIÉ par écrit, sinon le gate de `frontmatter.test.js` rougit (volet inverse compris : une justification périmée rougit aussi). ⚠️ Il a trouvé `note` absent de `defaults` au 1er run, après deux jours passés dans ce fichier — **l'œil ne voit pas les trous de symétrie, une machine oui**. ⚠️ **`confirm` RETIRÉ le 05/08/2026** : le vocabulaire est désormais INTÉGRALEMENT symétrique et `ASYMETRIES_JUSTIFIEES` est VIDE. Il était global à `false` (donc mort sans que personne le voie), non supporté par Codex, et contraire au 0-human. NE JAMAIS le réintroduire — `enforce` couvre le besoin.
8. **Croissance = enrichir le VOCABULAIRE** (primitives composables), jamais du calcul arbitraire.
   ⚠️ **UN CONCEPT = UN MOT, PARTOUT (loi anti-synonyme)** : avant d'ajouter une clé, vérifier si une primitive existante couvre déjà la sémantique — si la nouvelle clé alimente le MÊME chemin de code qu'une clé existante, c'est LA MÊME clé (la réutiliser, jamais la renommer par contexte). Un mot nouveau exige une SÉMANTIQUE nouvelle. Précédent réel : `perimeter` inventé comme synonyme de `match` pour les skills (18/07/2026) → supprimé. Docs, skills, futures sources : le MÊME vocabulaire (`match`/`mcp`/`rules`/`tool`/`scope`/`exclude`/`mode`/`threshold`/`driftUnit`/`note`/`enforce`), sans exception. ⚠️ **Un mot se RETIRE aussi** : `confirm`/`ask` supprimé le 05/08/2026 (390 frontmatters nettoyés, expand/contract) — une clé éteinte que personne n'utilise est une dette, pas une option. ⚠️ **`enforce` (05/08/2026)** = booléen, REFUSE le geste ; **`false` explicite ANNULE l'héritage** de `defaults.{source}` (sans lui, une catégorie passée en enforce serait INDÉSINSCRIPTIBLE — l'impasse de toute cascade). **Pas d'étage global** : un blocage global refuserait le 1er geste de chaque session, et un système qu'on subit finit débranché. 🛑 Ne PAS inventer de valeur « ne bloque pas » (j'ai tenté `warn` : un mot qui ne faisait rien, donc un synonyme de `false`). ⚠️ **`note` (04/08/2026) = commentaire d'AUTEUR, le SEUL champ que le moteur ne lit JAMAIS** : destiné à qui vient MODIFIER l'entrée (« pourquoi ce mode/ce scope »), invisible à l'injection car le frontmatter entier est retiré du corps. 🛑 JAMAIS le pourquoi d'un INVARIANT — celui-là reste dans le corps, sinon la règle dérive. Les 4 DÉCLENCHEURS, sémantiques DISJOINTES jamais fusionnées : `match` = substring CHEMIN · `mcp` = nom exact SERVEUR · `rules` = match par-entrée · `tool` (19/07/2026) = nom EXACT d'un OUTIL NATIF (WebFetch, WebSearch… — l'angle mort des outils sans chemin ni mcp__, comblé ; couvre d'office tout futur outil du harnais : le nommer dans une doc suffit, zéro code).
...
⚠️ **TROIS CHEMINS, ET LE 3ᵉ FERME TOUT (05/08/2026) : ça rentre ⇒ tel quel · ça déborde ⇒ morcelé sur N trames · ça déborde ENCORE ⇒ FILE, ré-émis au geste suivant.** La capacité d'UN appel reste finie (N × 7 658 = **91 896 c** au câblage actuel) parce que le harnais spawne exactement les trames déclarées — mais ce n'est plus un plafond de LIVRAISON, seulement un **DÉBIT** : le surplus attend en file au lieu d'être jeté, exactement comme l'émetteur TCP garde son tampon quand la fenêtre est pleine. `--paquets N` peut donc être BAISSÉ sans rien perdre.
✅ **LA RÉSERVE EST LEVÉE (05/08/2026, ⑯+⑮ LIVRÉS) — le transport est une COUCHE, plus un choix d'appelant.** `emission-core.js` est la SEULE voie par laquelle un contexte sort ; `porte-core.js` ET `session-inject.js` la traversent. La porte SESSION n'avait AUCUN transport (ni sceau, ni morcelage, ni file) et ne tenait que parce que `docs/session/` pesait ~1,2 Ko — du dimensionnement statique.
⚠️ **CE QUI REND LA COUCHE OBLIGATOIRE, C'EST LE GATE, PAS L'EXTRACTION** : `emission-core-gate.test.js` scanne les fichiers qui écrivent la clé `additionalContext` et exige qu'ils ATTEIGNENT le module (traversée TRANSITIVE, DÉRIVÉE du code) ⇒ tout émetteur FUTUR est couvert le jour où il est écrit. Dans un framework web on ne PEUT pas contourner le pipeline ; ici on possède tout, donc seule une MACHINE peut l'imposer. Negative-check par sabotage EN MÉMOIRE (jamais un fichier réel : la 1re version d'un tel check avait fait tomber 38 tests d'autres suites).
⚠️ **LA FILE EST PARTAGÉE ENTRE LES DEUX PORTES, ET C'EST LA CLÉ DE ⑮** : à SessionStart il n'y a pas de « geste suivant » où drainer un reliquat. Store commun (`reliquat-`, même scope d'agent) ⇒ ce que la porte session n'a pas pu livrer est repris par la porte PreToolUse au TOUT PREMIER appel d'outil. Zéro modification de `settings.json`.
⚠️ **RESTE UNE LIMITE HONNÊTE, PAS UNE RÉSERVE** : la porte session émet sur UNE trame (`nbPaquets: 1`) — qu'un hook SessionStart déclaré N fois soit spawné N fois n'est PAS mesuré. À une trame le morcelage livre quand même TOUT, simplement sur plusieurs gestes. Passer à N = un réglage APRÈS mesure, jamais une reconception.
**Sur TOUTES les voies (docs fichier, docs MCP, skills, docs de session), l'indélivrabilité est désormais impossible :** une doc de n'importe quelle taille arrive — 10 Ko, 80 Ko, peu importe. **NE JAMAIS réintroduire un plafond de taille, un « trop gros », un « scinde ta doc ».** Ce serait faire porter à l'AUTEUR d'une doc un défaut du TRANSPORT. Le framework livre, il ne juge pas ce qu'on lui confie. C'est pour ça que le volet ④ du `couverture-gate` (plafond de longueur) a été SUPPRIMÉ, et que la règle « <10 lignes » n'est qu'une convention de parc — jamais une contrainte du moteur.
...
🔴 **LE BUDGET DU MOTEUR DOIT SUIVRE LA LIMITE DÉCLARÉE — sinon VERT QUI MENT (⑰, 05/08/2026).** On déclarait `additionalContextLimit = 0` à Codex depuis le 04/08 (= *« disables spilling »*, donc AUCUNE limite) **sans jamais le dire au moteur** : plancher 8 000 appliqué, skill de 76 000 c livré en **11 gestes au lieu d'1**, avec 995 tests verts, mutation 100 %, doctor 27/27, canari vivant. Ce n'est PAS une panne, c'est une **DÉGRADATION SILENCIEUSE** — et **rien dans le repo ne mesure le DÉBIT** (⑱). FIX : `--budget N` dans la commande, à côté de la limite, MÊME bloc, `budget-declare-gate` exige l'égalité. **CLASSE D'ERREUR : tout ce qu'on DÉCLARE à un harnais doit être RELU par le moteur, jamais deviné en parallèle.**
⚠️ **MESURER LE BINAIRE, PAS SEULEMENT LA DOC** : `additionalContextLimit` = **0 occurrence en 0.144.6**, **18 en 0.146.0**. Notre déclaration était INERTE pendant deux jours. Une clé documentée n'est pas dans le binaire installé. ⚠️ **Codex 0.146 stocke ses transcripts en `~/.codex/sessions/AAAA/MM/JJ/rollout-*.jsonl`** (`logs_*.sqlite` = télémétrie, pas un transcript).
...
⚠️ **Un reliquat = un DÉLAI, jamais une perte NI une erreur de config (corrigé 05/08/2026).** L'ancien message accusait `--paquets N` et réclamait une intervention humaine pour un phénomène NORMAL de transport — c'est le toil qu'on supprime. Il attend en file et repart au geste suivant. Câblage actuel : **N = 12**, et ce nombre n'est plus dimensionné sur « le plus gros contenu connu » (capacity planning statique = bricolage) : il ne règle que la VITESSE.
⚠️ **TROIS GARDE-FOUS SOLIDAIRES, sinon la file BOUCLE À L'INFINI** (défauts mesurés en construisant, invisibles tant qu'on jetait le reliquat) : ① l'annonce compte des **DOCUMENTS** (dédup par label) et en cite **5 max** ; ② **garantie de progrès** — si rien ne tient, on force UN morceau et on sacrifie l'annonce (*livrer passe avant décrire*) ; ③ `morceler` abandonne l'en-tête de morceau quand la capacité ne le porte pas. En retirer un = boucle infinie ou trame hors budget.
⚠️ **`n === 1` MORCELLE AUSSI** : le court-circuit « une trame ⇒ rendu de `planifier()` » était un TROU (planifier ne morcelle pas) ⇒ sur Codex, une doc trop lourde n'arrivait JAMAIS. **Les 2 harnais ont désormais la même garantie**, Codex avec un débit plus faible.
...
- **TROIS décisions, jamais quatre : `none` · `allow` · `deny`.** L'INJECTION informe par DÉFAUT ; `enforce: true` est l'EXCEPTION DÉCLARÉE qui refuse le geste (05/08/2026). ⚠️ **Le fait qui a rouvert le sujet** : doc officielle, l'`additionalContext` d'un PreToolUse arrive *« next to the tool result »* ⇒ **une injection ne peut PAS empêcher le geste qu'elle vise**, elle protège le suivant. L'incident FONDATEUR (clic de paiement) n'aurait donc PAS été évité par une doc injectée — seul un refus le fait. `deny` = même JSON sur les 2 harnais, **zéro interaction utilisateur** (*« fully automatic »*), vérifié DANS le binaire Codex 0.144.6. ⚠️ **`ask`/`confirm` RETIRÉS du framework le 05/08/2026** (escalade humaine = anti 0-human ; absent de Codex donc deux sens pour un mot ; `enforce` couvre le besoin). `WRITE_TOOLS` est parti avec — plus aucune décision ne dépend du NOM de l'outil. Anti-retour scellé dans gate.test.js + spawn réel sur les 2 coquilles : NE JAMAIS le réintroduire. 🛑 **Ce n'est PAS une sécurité mais un GARDE-FOU** : la porte est fail-open, un hook mort laisse passer — protège d'un agent distrait, jamais d'un adversaire.
- **ANTI-BOUCLE = ALTERNANCE (l'idée du mainteneur, meilleure que la mienne)** : un blocage n'est JAMAIS suivi d'un blocage ⇒ le geste refait passe TOUJOURS, puis la cadence reprend. Donc **`enforce` SUIT la cadence et AUCUN mode n'est interdit** : `once` = un blocage par session · `smart` = rebloque après `threshold` appels (l'unité vient de `driftUnit`) · `dumb` = bloque/passe/bloque. ⚠️ J'avais d'abord interdit `smart`, puis exigé `once` explicite, puis interdit `dumb` — **trois restrictions pour un problème inexistant**, remplacées par UN drapeau d'état. Ne JAMAIS les réintroduire.
- **`doc-write-guard`** (PostToolUse) bloque sur doc INVALIDE — correcteur d'écriture, pas bloqueur d'action.
