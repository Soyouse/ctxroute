...
`CTXROUTE_LOCK_TIMEOUT_MS` = env RÉSERVÉE AUX TESTS (19/07/2026, doctrine paths.js) : les tests de concurrence relèvent le timeout pour prouver l'ATOMICITÉ indépendamment de la charge (2 s expirent légitimement sous charge = fail-open voulu, pas un bug). JAMAIS un réglage utilisateur — prod = 2000 ms.
