...
⚠️ **UN COMPTE SYSTÈME/CI N'EST PAS UNE IDENTITÉ** : sur GitHub Actions le compte s'appelle `runner` — dérivé tel quel il matchait « test runner », « tap-runner », « commandRunner » ⇒ **13 faux positifs, CI ROUGE au premier push**. Les comptes génériques (`runner`, `root`, `user`, `build`, `ubuntu`…) sont ÉCARTÉS quelle que soit leur provenance. Corollaire : **un terme dérivé de l'environnement peut être un mot courant sur une AUTRE machine** — le vérifier sur la CI, jamais seulement en local.
