...
// L'état (spawns séquentiels des hooks) est construit au niveau module —
// l'ordre d'exécution d'origine du harnais est préservé.
...
// ── Fixture : config de test JETABLE (tmpdir) — rien à sauvegarder/restaurer ──
...
// ── Cleanup synchrone (avant la partie async — même position que le harnais d'origine) ──
...
// ⚠️ `retry: 2` — même contrat que le test concurrence de doc-inject.test.js :
//    sous charge extrême, le timeout du lock (2 s) déclenche le FAIL-OPEN VOULU
//    (1 écriture sautée = disponibilité > état). Un VRAI bug de lock échoue les
//    3 tentatives. Ne jamais élargir ce retry ni toucher au timeout du lock.
// ⚠️ Signature vitest : avec un objet d'options, le timeout DOIT être DANS les
//    options — un timeout en dernier argument est IGNORÉ (piège vécu : mort à 5 s).
test(`concurrence : ${N_CALLS} appels parallèles sur ${N_SERVERS} serveurs → AUCUNE écriture perdue (lock cross-process)`, { retry: 2, timeout: 60000 }, async () => {
  setConfig({ mode: 'dumb', defaultThreshold: 4, servers: {} });
