...
// ⚠️ Chemins RÉSOLUS À CHAQUE APPEL via paths.js (SOURCE UNIQUE) — jamais
// recopiés ici. Résolution paresseuse OBLIGATOIRE : figer ces valeurs dans
// des const au chargement du module casserait l'isolation des tests/doctor
// (env vars posées par le parent, lues au spawn).
...
//    Claude Code sur Windows ne ferme pas toujours le stdin du hook qu'il spawne
//    (bug Anthropic #68626) : sans ça, ce process attend un `end` qui n'arrive
//    jamais et vit POUR TOUJOURS. Mesuré le 15/07/2026 : 875 zombies, un de 20 h,
//    0,8 Go de RAM libre sur 16. `.unref()` garantit ZÉRO latence ajoutée quand
//    tout va bien. Gate : `deadline-gate.test.js`. Preuve : `deadline.test.js`.
deadline.arm();
