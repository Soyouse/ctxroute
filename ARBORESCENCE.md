...
- `Desktop/mcp-doc-hooks/budget.js` — BUDGET D'ÉMISSION, PUR, muté 100 % : **LIVRE TOUT** (03/08/2026). Deux chemins et pas trois — ça rentre ⇒ tel quel ; ça ne rentre pas ⇒ MORCELÉ (`morceler`) et réparti sur N trames, protocole RFC 2046/6455. L'indélivrabilité est impossible par construction ; budget trop petit pour l'enveloppe ⇒ on DESCELLE et on livre. Ne connaît AUCUN seuil de harnais (le chiffre vient de la coquille).
...
- `Desktop/mcp-doc-hooks/canari.js` — CANARI D'INJECTION (03/08/2026), PUR, muté : tranche `vivant`/`mort`/`indecidable` sur ce qui a RÉELLEMENT atterri dans le transcript du harnais. Le SEUL témoin qui regarde l'autre bout du tuyau — tout le reste se teste soi-même et resterait vert si le harnais changeait de contrat.
- `Desktop/mcp-doc-hooks/canari-check.js` — coquille I/O du canari (UserPromptSubmit, MUETTE par contrat) : lit la QUEUE du transcript (2 Mo, jamais tout — 104 Mo mesurés dans le parc) et écrit `state/canari.json`. Sur erreur : n'écrit rien, le verdict précédent survit.
- `Desktop/mcp-doc-hooks/canari.test.js` — suite DÉTERMINISTE du canari (celle que Stryker mute) : verdict, frontière du seuil, totalité, comptage, negative-check.
- `Desktop/mcp-doc-hooks/canari-check.test.js` — suite par SPAWN RÉEL de la coquille : canal sain, canal MORT, mutisme du stdout, lecture bornée, préservation du verdict sur erreur.
- `Desktop/mcp-doc-hooks/deps-purete-gate.test.js` — MÉTA-GATE : vérifie que les règles `*-must-stay-pure` PEUVENT rougir. Né d'un bug réel (03/08/2026) où `lib-pure-must-stay-pure` était INERTE — `includeOnly` écartait les modules cœur du graphe, donc aucune règle ne pouvait les voir.
