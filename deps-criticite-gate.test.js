...
//
// ⚠️ POURQUOI ICI (propagation parc, 30/07/2026) : le gate est né dans un autre repo du parc après une asymétrie
...
// ⚠️ Les RÈGLES vivent dans `deps-criticite-pure.js` (mutées par Stryker) — un gate ne fait que CONSTATER.
//    Une décision enfermée dans un fichier de test n'est jamais mutée, donc jamais prouvée.
import { estEpingleExact, fautesEpinglage as fautesPures, depsNonClassees, entreesFantomes } from "./deps-criticite-pure.js";

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)));

// ⚠️ BOM TOLÉRÉ À LA LECTURE — trouvé en posant ce gate : `package.json` portait un BOM UTF-8 (un éditeur
//    Windows), et `JSON.parse` LÈVE dessus alors que npm, lui, l'accepte. Résultat : l'outillage qui lit
//    le manifeste meurt sur un fichier que le gestionnaire de paquets trouve parfaitement valide. Le BOM a
//    été retiré, mais un gate ne doit pas MOURIR sur le défaut qu'il est censé rapporter.
...
describe("criticité des dépendances", () => {
  ...
  test("toute dépendance classée `moteur` est ÉPINGLÉE EXACT (ni ^, ni ~, ni plage)", () => {
    const fautes = fautesPures(toutesDeps(), MANIFESTE.moteur).map((d) => `${d.ou} · ${d.nom} = "${d.plage}" — classé MOTEUR ⇒ DOIT être épinglé EXACT.`);
    expect(fautes, `\n${fautes.join("\n")}\n`).toEqual([]);
  });
