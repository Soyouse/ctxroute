...
**`d6125b4` la tête était PÉRIMÉE** (elle s'arrêtait 9 commits en arrière, et ㉙ y figurait
...
| ㉘ bis | **Marque `[source:` auto-référente** — ✅ **FERMÉ 08/08/2026, ET LA « FENÊTRE CALME » ÉTAIT UNE FAUSSE CONTRAINTE.** Cette ligne annonçait un fix dans `emission-core`, donc le chemin CHAUD (12 processus × chaque appel d'outil × chaque agent), donc l'attente d'un moment où personne ne travaille. 🛑 **La mesure a démoli la prémisse** : les « 4 docs qui CITENT un `.md` » ne citaient rien — elles portaient le tag `[source: …]` **COLLÉ EN DUR**, un copier-coller d'injection (le moteur l'ajoute déjà ⇒ elles arrivaient avec leur tag EN DOUBLE, et un agent qui les LISAIT déposait une étiquette valide dans le transcript ⇒ canari VERT sur canal potentiellement mort). ✅ Livré : 4 lignes retirées, **zéro ligne de moteur**, aucun agent perturbé. Anti-retour = règle de lint `tag-source-en-dur` (ERREUR — le tag ne s'écrit jamais à la main, donc zéro exemption légitime). ⚠️ **AU PREMIER RUN SUR LE PARC RÉEL, MON MOTIF A ACCUSÉ `canari.md`** — la doc qui EXPLIQUE le marqueur : resserré sur une LIGNE ENTIÈRE (la forme qu'émet le moteur). *Une doc qui PARLE du mécanisme n'en porte pas.* 🛑 **LEÇON, plus large que ce chantier** : le backlog portait pendant 24 h un fix moteur coûteux ET une contrainte d'ordonnancement, pour un défaut que 4 corrections de TEXTE suppriment. Le contrat d'extension le disait déjà — *« un trou se règle d'abord en DONNÉES, le moteur en DERNIER recours »* — et je ne l'ai relu qu'après que le mainteneur a demandé **« pourquoi une fenêtre calme ? »**. **Une contrainte héritée d'une session précédente n'est pas une mesure : la re-mesurer avant de s'y plier.** | ✅ |
...
| ㉜ | **㉚ NE VOIT PAS UNE TÊTE INCOMPLÈTE** — prouvé le 08/08/2026 : elle annonçait 18 commits, il y en avait **25**, et le gate était VERT (il compare N annoncé à N empreintes CITÉES = cohérence INTERNE, jamais l'exhaustivité). 🛑 Trouvé UNIQUEMENT parce que le mainteneur a redemandé « tu es sûr à 100 % ? ». ✅ **FERMÉ 08/08/2026 — volet ③ de `backlog-coherence-gate`, EXERCÉ EN RÉEL** (rouge constaté sur le dépôt vivant en citant volontairement un commit de moins, puis vert). 🛑 **LA PISTE ÉCRITE ICI LA VEILLE ÉTAIT LA MAUVAISE** : « dériver les commits DU JOUR ». Mesurée sur 4 jours réels ⇒ 0 · 6 · 16 · 24 manquants — et ces rouges sont FAUX, car une session de NUIT traverse minuit et la tête groupe légitimement deux dates. **« Le jour » n'est pas décidable ici.** ✅ Ce qui marche : une PLAGE, sans calendrier — de l'édition PRÉCÉDENTE de la tête jusqu'à `HEAD`, la tête datant elle-même sa fraîcheur. Mesuré sur les 11 dernières éditions : **9 vertes, 2 rouges, les 2 rouges étant de VRAIES omissions** (`52ec4c5` oubliait les 4 commits de `rank`, `f3f8dcd` oubliait `c9b3dcf`) — zéro faux positif. ⚠️ Ne tire QUE si le fichier est MODIFIÉ dans l'arbre : sans ce garde-fou il exigerait de réécrire la tête à chaque commit, donc viserait le travail normal au lieu de l'oubli. 🛑 Muet sur clone superficiel (la CI n'a pas d'historique) — **local par nature, et c'est le bon endroit : la tête s'écrit en local, jamais en CI.** | ✅ |
| ㉝ | **`master` exige une PR, et tous les pushes de l'agent la CONTOURNENT** (constaté 08/08/2026 : `remote: Bypassed rule violations — Changes must be made through a pull request`). Passe parce que le compte a le droit de contourner. **Décision du mainteneur requise** : soit retirer la règle (elle ne protège personne si elle est contournée à chaque push), soit basculer l'agent sur des PR. 🛑 Une règle qu'on contourne systématiquement est PIRE que pas de règle : elle donne l'illusion d'un garde-fou. ✅ **APPLIQUÉ 08/08/2026** (décision déléguée par le mainteneur) : la règle `pull_request` a été RETIRÉE du ruleset `master-proprietaire-seul` (19173835), **`deletion` et `non_fast_forward` CONSERVÉES**, enforcement toujours `active` — vérifié par API. 🛑 **Le choix n'est pas « moins de protection »** : les deux règles gardées protègent contre l'IRRÉVERSIBLE (suppression de branche, réécriture d'historique) et **ne sont contournées par personne** ; la PR ne protégeait de rien sur un dépôt à un seul auteur où l'agent a le droit de bypass — elle ne produisait qu'une ligne `Bypassed rule violations` à chaque push, c'est-à-dire un garde-fou décoratif. | ✅ |
| ㉟ | **CLASSE : UN GATE DÉRIVÉ EST AVEUGLE AUX ÉTATS QU'IL NE DÉRIVE PAS** (audit du 08/08/2026). Trois occurrences le MÊME jour, et c'est ce qui en fait une classe et non trois incidents : ① `couverture-gate` dérive des fichiers **TRACKÉS** ⇒ un fichier neuf passe 4/4 tant qu'il n'est pas commité (mesuré : `differential-normalise.js` a caché 2 manques réels, dont l'absence d'`includeOnly` — le volet « le plus traître ») · ② la **mutation INCRÉMENTALE** ne rejoue pas ce que le cache couvre ⇒ 7 survivants invisibles sur `canari.js` (= ㉞) · ③ aucun gate ne voit **la couche** où une logique est posée : l'ordinal placé dans `budget.js` (transport) au lieu de `porte-core.js` (identité) n'a été attrapé que par un test d'attente exacte, par CHANCE. 🛑 **NE PAS traiter en 3 correctifs séparés** — la cause est commune : *ce dont un gate tire sa liste définit son angle mort*. ⚠️ **MESURER AVANT D'ÉCRIRE** (règle du repo, et le bruit est le risque réel ici : inclure les non-suivis ferait rougir sur des brouillons). Aucun de ces trois n'est bloquant ; les trois sont des VERTS QUI MENTENT, ce que ce repo combat en priorité. | 🟠 |
| ㉞ | **LA MUTATION INCRÉMENTALE MASQUE DES SURVIVANTS — mesuré 08/08/2026.** Passe COMPLÈTE (worktree neuf, donc SANS `stryker-incremental.json`) : score global **99,64 %**, mais **`canari.js` à 89,23 % avec 7 survivants** — alors que le backlog affirmait « canari.js 100 % / 0 survivant ». 🛑 **Personne n'a touché `canari.js`** : c'est le CACHE qui ne rejouait plus ces mutants. Hypothèse VÉRIFIÉE (le fichier incrémental existe côté prod, absent du worktree). Le seuil de rupture (99) reste tenu, donc rien n'est bloqué — mais c'est un **vert qui ment** depuis une date inconnue, exactement la classe que le repo combat. ✅ **FERMÉ 08/08/2026 — ET MA PISTE ÉTAIT FAUSSE SUR LES DEUX PLANS.** ① « une passe complète périodique » : **elle existe déjà** — `mutation.yml` fait `npm ci` sur un clone vierge et ne restaure AUCUN cache (`incremental` : 0 occurrence dans le workflow) ⇒ la CI mute TOUT à chaque push touchant un module muté. Le faux vert était **LOCAL**, jamais en CI. ② Et pourtant `canari.js` est resté à 89,23 % **sans que la CI bronche** : `thresholds.break` de Stryker est **GLOBAL**, donc 99,64 % de moyenne noyaient l'effondrement. **Ce n'était pas la FRÉQUENCE des passes qui manquait, c'était la GRANULARITÉ du verdict** — soit très exactement la classe ㉟ appliquée à un seul nombre agrégé. ✅ Livré : `mutation-plancher-gate.test.js` (plancher **PAR FICHIER**, complète `break` sans le remplacer). ✅ Et les 4 derniers survivants ont été **ÉLIMINÉS, pas testés** : c'étaient les 4 mutants de `occurrences()`, une fonction **sans aucun appelant et non exportée**, reste du canari d'avant le 07/08 — la couvrir aurait figé du code inutile pour toujours. **Résultat mesuré, passe COMPLÈTE (cache supprimé) : 100,00 % sur les 16 modules, 0 survivant** ⇒ plancher fixé à 100, **zéro exemption**. 🛑 Un plancher qu'il faut assortir d'exceptions dès le premier jour est un plancher qu'on abaissera au deuxième. | ✅ |
| ㉛ | **`doctor.js` écrit un JSON sans tmp + `rename`** — ✅ **VÉRIFIÉ ET CLOS 08/08/2026 : le risque N'EXISTE PAS, par construction.** Le seul JSON concerné (`reliquat-doctor-probe.json`) vit dans `stateDir = path.join(tmp, 'state')` où `tmp` vient de `fs.mkdtempSync` — un dossier **privé à cette invocation du doctor**, jamais le `state/` vivant. Et son unique lecteur est le canari que la sonde lance elle-même en **`spawnSync`**, donc APRÈS que l'écriture soit terminée : il n'existe aucun lecteur concurrent, pas même en théorie. 🛑 **Ne PAS « corriger » par symétrie avec `session-store`** — là-bas les lecteurs concurrents sont réels et mesurés (4 779 lectures creuses) ; ici l'écriture atomique n'ajouterait qu'un rename inutile et un faux message de sécurité. Le motif ne fait pas le défaut : c'est le LECTEUR CONCURRENT qui le fait. | ✅ |
